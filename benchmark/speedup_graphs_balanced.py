...
csv_filename = f"speedup_data_balanced.csv"  # Store data in CSV
        ...
        for thread in THREADS:
            ...
            for i in range(RUNS):
                op = (
                    (
                        subprocess.check_output(
                            ["go", "run", "editor.go", "balanced", mode, thread],
                            cwd="../editor",
                            text=True,
                        )
                    )
...
plt.ylabel("Speedup")
plt.title(f"Speedup vs Threads - Balanced")
plt.legend()
plt.grid(True)

plt.savefig("./speedup-images-balanced.png")
